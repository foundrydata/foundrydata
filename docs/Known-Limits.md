...
- **Dimensions & denominators**: Coverage dimensions (`structure`, `branches`, `enum`, `boundaries`, `operations`, …) are projections over the same target universe. `dimensionsEnabled` controls which dimensions are reported, not which targets exist; disabling a dimension hides its metrics but does not change `CoverageTarget.id` values. `excludeUnreachable` affects only coverage denominators by removing `status:'unreachable'` targets from ratio calculations; unreachable targets remain present in `targets` / `uncoveredTargets` with stable IDs and statuses.
