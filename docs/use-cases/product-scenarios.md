...
foundrydata openapi \
  --spec examples/openapi/users-api.json \
...
```bash
npx tsx examples/api-mocks.ts
```

This script loads `examples/openapi/users-api.json`, selects the `getUsers` 200 JSON response, calls `Generate` with a fixed seed, validates a sample of responses with `Validate`, and prints both a short summary and a handful of example payloads.
...
foundrydata generate \
  --schema examples/schemas/payment.json \
  --n 10 \
  --seed 123 \
  --out ndjson
```
...
```bash
npx tsx examples/contract-tests.ts
```

This script loads `examples/schemas/payment.json`, runs `Generate` with a fixed seed to produce a small list of payments, validates them with `Validate`, and prints a summary that can be asserted from an integration test (e.g. “10 items, all AJV-valid, deterministic for seed 123”).
...
foundrydata generate \
  --schema examples/schemas/llm-output.json \
  --n 5 \
  --seed 99 \
  --out ndjson
```
...
```bash
npx tsx examples/llm-output.ts
```

This script loads `examples/schemas/llm-output.json`, calls `Generate` with a fixed seed to produce a few structured outputs, validates them via `Validate`, and prints both a small sample and a summary that could be asserted from unit tests or used to eyeball how “realistic” the generated shapes feel.
