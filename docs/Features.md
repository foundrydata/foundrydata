...
| `multipleOf` / numeric bounds | ~ | Numeric bounds contradictions are detected in Compose and surface as fatal `UNSAT_NUMERIC_BOUNDS` diagnostics when the real or integer domain collapses (see `docs/error.md`); runs fail early in that case. Generation snaps values to `multipleOf` with an epsilon derived from `rational.decimalPrecision`, and rational caps are enforced but do not yet emit the `RAT_*` diagnostics, so these fallbacks remain AJV-valid but are only partially observable. |
...
| `$dynamicRef` / `$dynamicAnchor` / `$recursiveRef` | ~ | Normalizer and Compose keep `$dynamic*` keywords in the schema but record diagnostics when they are present: `DYNAMIC_PRESENT` notes the presence of dynamic scope, and `DYNAMIC_SCOPE_BOUNDED` is emitted when a bounded in-document binding is found within `guards.maxDynamicScopeHops` (default `2`; see `docs/Known-Limits.md` and `docs/error.md`). During generation, the engine may resolve `$dynamicRef` to that bounded target but never crosses documents; when no bounded binding is available (including when the hop limit is exceeded), no substitution occurs and behavior remains a pass-through to AJV. There is no dedicated exit code for dynamic-scope behavior. |
...
| Capability | Status | Implementation notes |
| --- | --- | --- |
| Coverage measurement (structure/branches/enum) | ✓ | `coverage=measure` mode is implemented for JSON Schema and OpenAPI entrypoints; it materialises coverage targets for structure, branches and enums while keeping the generated instances identical to coverage=off for a fixed determinism tuple (canonical schema, OpenAPI spec?, coverage options incl. `dimensionsEnabled`/`excludeUnreachable`, seed, AJV posture, resolver `registryFingerprint`; see `docs/Invariants.md`). Metrics and coverage-report/v1 are available, and diagnostic-only targets (e.g. SCHEMA_REUSED_COVERED) are excluded from denominators and thresholds. |
...
- A minimum overall coverage threshold (`minCoverage`) applied to the enabled dimensions, enforced via coverage-report/v1 `metrics.coverageStatus` and the dedicated coverage threshold exit code (`COVERAGE_THRESHOLD_NOT_MET`, exit code `70`; see `docs/error.md` and `packages/core/src/errors/codes.ts`). In practice, teams typically tune `minCoverage` by schema size and risk, or gate on regressions (coverage drops vs. `main`) plutôt que de s’appuyer uniquement sur un seuil absolu.
