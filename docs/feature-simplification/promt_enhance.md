...
When a proof or behavior depends on JSON Schema/AJV semantics and cannot be conclusively derived
from SPECIFICATION INPUT, raise an issue (type:"ambiguity") with a concrete risk and do not assume behavior.
...
• Always cite exact sections (§/anchors). For line ranges use stable approximations:
  format "§<anchor> · bullet <n>" or "§<anchor> · para <n>" (counting top‑level bullets/paragraphs).
  Do not invent absolute line numbers.
...
• AJV as oracle: all generated instances validate with AJV against the ORIGINAL schema,
  except the documented Lax exception in the SPEC (§1/§11): when Source AJV compile
  fails solely due to unresolved external $ref, skip final validation and emit
  EXTERNAL_REF_UNRESOLVED with details.skippedValidation=true and
  diag.metrics.validationsPerRow=0.
• Determinism: same seed ⇒ same outcomes; no global mutable state; deterministic branch selection (including score‑only).
• Pipeline & responsibilities: Normalize → Compose → Generate → Repair → Validate; final validation uses the original schema,
  with the Lax-mode skip path above.
• Observability: stable diagnostic codes/payloads; metrics; explicit degradations when caps apply.
• Config gate (§13 parity): both AJV instances satisfy required flags (incl. unicodeRegExp:true; aligned multipleOfPrecision when fallbacks are used). Any deviation ⇒ AJV_FLAGS_MISMATCH (hard failure).
...
• Reproducible excerpts: the ≤200‑char excerpt MUST start at the first full sentence (or bullet text)
  that contains the quoted term; trim on Unicode code‑point boundaries (never split surrogate pairs);
  no ellipsis mid‑token.

POLICY (priorities & budgets)
• Priorities: Correctness > Determinism > Simplicity > Performance > Features.
• p95 budgets: diag.metrics.p95LatencyMs ≤ 120 ms, diag.metrics.memoryPeakMB ≤ 512 MB. CI MUST fail when either bound is exceeded (see §1/§15). compile_time ≤ 1000 ms is a tracked SLI (non‑blocking).
• On degradation or skip‑trials: emit diagnostics; show deterministic score‑only path with diag.budget.skipped=true, diag.budget.tried=0, appropriate reason, and record scoreDetails.tiebreakRand even when |T|=1. At any branch node, always include scoreDetails.orderedIndices and topScoreIndices (even if branches.length===1).
...
• Reuse existing diagnostic codes from §19. If a new code is strictly needed for documentation only (no behavior expansion),
  you MUST include a patch that adds it to §19 with a normative payload and update cross‑references in your patches.
  Otherwise, do not mint new codes.
...
4) Patch rationale & impact (concise): for each patch, state why it is needed, what it fixes (correctness/determinism/observability), and confirm “no behavior expansion”.
5) Cross‑refs & diagnostics alignment: list sections and diagnostics impacted; confirm all referenced codes exist in §19 and payloads match §19.1.
6) Assertions list (MANDATORY): 8–15 atomic, verifiable statements that the patched spec must guarantee (not executable tests). Format each as:
   ...
   • External $ref — Strict vs Lax behavior; no remote deref; required diagnostics.
   • Property order — required first, then optional; both in UTF‑16 lex order.
   • Pattern witness — anchored‑safe only; complexity‑capped treated as non‑anchored; ≤1 key per pattern per pass; shortest length then lexicographic order.
   • Regex failures — compile errors under /u are unknown gating with REGEX_COMPILE_ERROR; do not trigger fail‑fast by themselves.
7) Scoring (GLOBAL) — deliver a viability score for the unmodified SPECIFICATION INPUT (exact bytes as received):
   • GSV — Global Spec Viability (0–100), computed on the unmodified SPECIFICATION INPUT. Do not factor your patches, notes, or assertions.
   • Confidence (0–1).
   • Rubric breakdown (must sum to GSV):
     – Correctness (0–30) — JSON Schema 2020‑12/AJV conformance; AJV‑as‑oracle; sound early‑unsat.
     – Determinism (0–30) — seeded behavior; stable Top‑K; RNG evidence (`tiebreakRand`); memo keys include (canonPath, seed, AJV.major, AJV.flags, PlanOptionsSubKey incl. ε).
  ...
  "issues": [
    {
      ...
      // Exactly one of the two forms above. Line numbers (Lx–Ly) are disallowed for stability.
      "excerpt": "<<=200 chars (must contain the first occurrence of the ISSUE TERM)>>"
      // ISSUE TERM (in priority order): the diagnostic code (e.g., AP_FALSE_UNSAFE_PATTERN),
      // else the target §anchor id, else the section title keyword.
...
C) Determinism — seeded RNG for ties and score‑only; stable Top‑K (score desc, index asc); memo keys include (canonPath, seed, AJV.major, AJV.flags, PlanOptionsSubKey with ε).
...
F) External $ref — Strict vs Lax; no remote deref; diagnostics.
G) Complexity caps — deterministic degradation; required diagnostics; score‑only determinism preserved.
...
L) Early‑unsat under AP:false — provable vs approximate paths with correct diagnostics (e.g., UNSAT_REQUIRED_AP_FALSE, UNSAT_AP_FALSE_EMPTY_COVERAGE, AP_FALSE_INTERSECTION_APPROX).
...
• When citing “branch selection — score‑only…”, verify that tiebreakRand is recorded even when |T|=1
  and that orderedIndices/topScoreIndices are present when branches.length===1; if missing, file as determinism issue.

OUTPUT ORDER
1. Summary
2. Issues JSON
3. Patches (diffs)
4. Patch rationale & impact
5. Cross‑refs & diagnostics alignment
6. Assertions list
7. Scoring (Global)
