 ...
 * **Objects — `additionalProperties` exactness**
  ...
  **Coverage-only anchored-subset lifting (non-anchored patterns).**
    **Preference order (normative):** try a **strict** subset first, then fall back to **substring** when strict is not applicable.
    **Strict detectors (normative):**
      1) **Exact-literal alternatives.** If `p` is a top-level alternation of **pure literals** (e.g., `foo|bar|baz`, `(?:foo|bar)`), with all metacharacters escaped and **no** character classes, quantifiers, or nested groups that change semantics, the implementation **MUST** lift to `^(?:L1|...|Lk)$` (anchored-safe), where each `Li` is the literal alternative (JSON-unescaped) with metacharacters escaped. This strict form is a **subset** of the original search semantics, hence **safe** for coverage.
      2) **Simple class quantified.** If `p` is a concatenation of character classes possibly followed by **short quantifiers** (`+`, `?`, `*`, `{m}`, `{m,}`, `{m,n}` with `0 ≤ m ≤ n ≤ 64`) and contains **no** look-around or back-references, the implementation **MUST** lift to the strictly anchored form `^p$` (anchored-safe), using the JSON-unescaped source with its quantifiers unchanged.
    **Substring fallback (unchanged):** When strict lifting does not apply but `p` has no look-around/back-references and is not complexity-capped, the implementation **MAY** use the textually anchored **substring-equivalent** `^.*(?:p).*$` for coverage-only proofs.
    **Guards (unchanged & normative):** patterns that fail to compile under `u`, contain look-around/back-references, or are flagged by the regex complexity cap **MUST NOT** be lifted and remain unknown gating.
    **Diagnostics (normative):** When a lift is applied, implementations **SHOULD** record `usedAnchoredSubset:true` and `anchoredKind:'strict'|'substring'` in approximation hints when relevant, and **MAY** include a coverage certificate under `planDiag.details.safeProof` when the safe-only proof is non-empty.
