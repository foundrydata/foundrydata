...
* For every SPEC task (9100..9124), the retrieval descriptor in details MUST resolve at runtime from a FILE:
  ...
  * "source":"FILE"
  * "uri":"docs/spec-canonical-json-schema-generator.md"
...
* Every logical SPEC pointer of the form `spec://§<n>#<slug>` maps to the physical markdown anchor:
  `docs/spec-canonical-json-schema-generator.md#s<n>-<slug>`
...
* Use exactly the slugs listed below for these references; **additional anchors MAY be used if present** in the pasted SPEC (no invented variants).

Common anchor slugs (reference)

* §7: spec://§7#object-keywords-pnames-rewrite
* §8: spec://§8#coverage-index-export, spec://§8#branch-selection, spec://§8#oneof-exclusivity, spec://§8#anchored-safe-definition, spec://§8#regex-complexity-cap
* §9: spec://§9#if-aware-lite, spec://§9#pattern-witness-selection, spec://§9#arrays-contains
* §10: spec://§10#mapping, spec://§10#propertynames-rename-guard, spec://§10#structural-hashing, spec://§10#epsilon-logging, spec://§10#property-order, spec://§10#mustcover-index-missing
* §11: spec://§11#external-ref-probe
* §13: spec://§13#startup-config-check
* §14: spec://§14#planoptionssubkey
* §19: spec://§19#envelope, spec://§19#payloads
...
* Format (string content is JSON; escape as needed for valid JSON):
  REFONLY::{"source":"FILE","uri":"docs/spec-canonical-json-schema-generator.md","section":"§<n>","anchors":["spec://§<n>#<slug>",...],"notes":"no-embed; fetch-on-demand"}
...
* Correct examples (copy-style):
  ...
  * SPEC task with two anchors (escaped for outer JSON string):
    REFONLY::{"source":"FILE","uri":"docs/spec-canonical-json-schema-generator.md","section":"§8","anchors":["spec://§8#coverage-index-export","spec://§8#branch-selection"],"notes":"no-embed; fetch-on-demand"}

  * SPEC task with three anchors (escaped):
    REFONLY::{"source":"FILE","uri":"docs/spec-canonical-json-schema-generator.md","section":"§10","anchors":["spec://§10#mapping","spec://§10#propertynames-rename-guard","spec://§10#structural-hashing"],"notes":"no-embed; fetch-on-demand"}
...
* Correct example inside `details` (escaped):
  [Retrieval] REFONLY::{"source":"FILE","uri":"docs/spec-canonical-json-schema-generator.md","section":"§22","anchors":["spec://§22#packages-core","spec://§22#scripts"],"notes":"no-embed; fetch-on-demand"}
...
* Priority policy (P0 → "high"): High for tasks {1,2,3,6,7,8,9,10,11,14,15,16,17,18,22}. Medium for {4,5,12,13,19,20,21,23,24}. (Adjust only if §16 in the pasted SPEC differs.)
...
* details (ENGLISH; REQUIRED sections, in this exact order). **When embedding a `[Retrieval] REFONLY::{...}` line here, escape the inner JSON as shown above.**
  ...
  [Deliverables] files to create/modify (per §22). Include the SPEC file path: docs/spec-canonical-json-schema-generator.md.
  ...
  **Diagnostics verbosity**: include a toggle to support “CI verbose” vs “runtime normal” without changing §19.1 shapes (apply in tasks #6 and #16).

* testStrategy: MUST list (i) unit checks for mandated diagnostics at this task's scope, (ii) integration/e2e where applicable, (iii) CI/bench expectations per §15–§16, **and (iv) property-based tests / fuzzing using AJV as oracle** for `oneOf` exclusivity, `AP:false` must-cover, `contains` bag, regex witness caps (cover in tasks #17 and #20 at minimum).
...
* Runtime mapping: spec://§<n>#<slug> → docs/spec-canonical-json-schema-generator.md#s<n>-<slug>.
...
* **At any branch node (`anyOf`/`oneOf`) (§8).**
  ...
  * `diag.scoreDetails` **MUST** be present (even when `branches.length===1`) and include:
    ...
    * `orderedIndices: number[]`, `topScoreIndices: number[]`,
    * `tiebreakRand: number | undefined` — **present** whenever RNG is used for selection and **always** in score‑only; may be `undefined` only when RNG was not used and trials occurred,
    * `exclusivityRand?: number` — **present when RNG is used in `oneOf` exclusivity step‑4`.
  * Record `diag.overlap.passing` and `diag.overlap.resolvedTo`.

* **`oneOf` exclusivity refinement (§8).**

  * **Numeric nudge**: integers → **±1**; non‑integers → **±ε** with **ε = 10^(−decimalPrecision)**.
  * **String tweak**: inject **U+0000** at end; if rejected, inject `"a"` at end. Apply only when the target value exists.
  * **Immediate re‑validation** against the **original schema** after each change.
  * **Determinism**: no RNG in step‑3; stable ordering; if `b*` no longer passes, use seeded RNG and record **`exclusivityRand`**.

* **Lax external `$ref` probe (§11).**
  Probe by replacing external `$ref` subtrees with `{}`; if the probe compiles and externals exist, **skip final validation**, emit `EXTERNAL_REF_UNRESOLVED{mode:"lax", skippedValidation:true}`, and set `diag.metrics.validationsPerRow = 0`. No network/filesystem I/O in any mode.

* **AP:false must‑cover + fail‑fast policy (§3, §8).**

  * **Fail‑fast `AP_FALSE_UNSAFE_PATTERN` (Strict) ONLY when** presence pressure holds, unsafe/capped patterns are **required** for proof, and the **Safe** set is **empty**.
  * Raw `propertyNames.pattern` never triggers fail‑fast; gating only.
  * Coverage may expand only via §7 rewrite with `PNAMES_REWRITE_APPLIED`.
  * In **Lax**, warn and proceed conservatively.

* **CoverageIndex (§8).**

  * `has(name)` is pure; `enumerate()` only when the global must‑cover intersection is provably finite from `properties` and/or **synthetic exact‑literal alternatives** (i.e., §7 rewrite present). Return UTF‑16‑sorted deduplicated names; include `provenance` (add `'propertyNamesSynthetic'` when rewrite contributed).

* **Property order (§9).**
  Required first, then optional; both in UTF‑16 lexicographic order.

* **Pattern witness search & caps (§9).**
  Normalize Σ by Unicode code point (dedupe; drop unpaired surrogates); bound by code points; iterate by length then UTF‑16; compile with `new RegExp(P,'u')`. Emit `COMPLEXITY_CAP_PATTERNS{...}` when capped; increment `patternWitnessTried` when metrics enabled.

* **Rationals & ε logging (§8, §10).**
  Log `details.epsilon:"1e-<decimalPrecision>"` for non‑integer exclusives and `multipleOf` snaps/tolerance checks; for integer exclusives you MAY log `details.delta:±1`.

* **Structural hashing (§10, §14).**
  SHA‑256 over canonical JSON (sorted keys, arrays in order, `-0→0`, BigInt stringified); confirm collisions via `deepEqual`.
...
* **Diagnostics envelope & phase separation (§19).**
  `{code, canonPath, details?}`; never duplicate `canonPath` inside `details`. Respect phase split.

* **MUSTCOVER guard in Repair (§10).**
  Under `AP:false` with guard active and CoverageIndex API absent: do **not** rename; emit `MUSTCOVER_INDEX_MISSING{guard:true}`.

* **Fuzzing / property-based tests (tasks #17 & #20).**
  Include schema/instance generators with AJV as oracle, covering `oneOf`, `AP:false` must‑cover, `contains` bag, regex witness caps.
...
12. Anchor resolution sanity: for every spec://§<n>#<slug> emitted, the markdown contains `<a id="s<n>-<slug"></a>` in docs/spec-canonical-json-schema-generator.md.
...
Default FILE path for runtime resolution: docs/spec-canonical-json-schema-generator.md.
...
* A single JSON array [...] with:
  ...
  • 1..24 (implementation tasks wired to SPEC via dependencies; priorities per policy above).
