...
* **Forced present (dependentRequired)** — A key is *forced present* in the **effective view** iff it is a member of
  the **effectiveRequiredKeys** union at that object **after** `allOf` merge. Presence that depends only on branch
  selection (`oneOf`/`anyOf`) or conditional activation (`if/then/else`) is **not** considered forced for early‑unsat
  and AP:false fail‑fast purposes.
...
6. **Object keywords**
   ...
   * `propertyNames` rewrite (only when strictly equivalent and when no `unevaluated*` is in scope). **MUST Preconditions (normative) for equivalence**:
   ...
   4) **AdditionalProperties safety:** `additionalProperties` at this object is absent, `true`, or `{}`. If `additionalProperties` is present and is **neither `true` nor `{}`** — i.e., it is `false` or a **non‑empty schema object** — **do not rewrite** and emit `PNAMES_COMPLEX` with detail `'ADDITIONAL_PROPERTIES_SCHEMA'`.
     ...
     * **Closed enum** form (additive canonicalization; **apply only when preconditions (1), (2) & (4) hold**):
       Given `{"propertyNames":{"enum":[n1,...,nk]}}`, **do not modify the original `propertyNames` node**. Construct a synthetic anchored‑safe alternation from the enum by first **deduplicating then sorting** `[n1,...,nk]` in UTF‑16 order (this ordering applies **only** to the alternation), escaping each literal via `escapeRegexLiteral`. **Add only** the following constraints in the **canonical view** (the original `propertyNames` remains as‑is):
       ```json
       {
  ...
  **Alternation order (normative):** the alternation `^(?:n1|...|nk)$` **MUST** list names in UTF‑16 lexicographic order after deduplication; each `ni` is escaped via `escapeRegexLiteral(ni)` before joining. This requirement constrains only the synthetic alternation; it **does not** reorder or deduplicate the original `propertyNames` array.
     ...
     **add** in the **canonical view** (retain the original `propertyNames` verbatim; do **not** duplicate it):
      ```json
      {
        "patternProperties": { "P": {} },
        "additionalProperties": false
...
* **Pattern execution engine (normative):** whenever the coverage/gate logic tests a `name` against a pattern
  ...
  **Severity routing (normative).** `REGEX_COMPLEXITY_CAPPED{context:'coverage'}` and `REGEX_COMPILE_ERROR{context:'coverage'}` **MUST** be recorded as **non‑fatal** entries in `diag.warn`. They **MUST NOT** appear in `diag.fatal` unless another rule independently escalates the node (e.g., §8 fail‑fast under AP:false).
  AJV remains the oracle at validation time; coverage analysis does not attempt to reinterpret such constructs.  
  **Normalizer cross‑reference (normative).** For §7 `propertyNames` rewrite preconditions, the **Normalizer** emits
  `REGEX_COMPILE_ERROR{patternSource: source, context:"rewrite"}` and `PNAMES_COMPLEX{reason:"REGEX_COMPILE_ERROR"}` and skips the rewrite. **Compose MUST NOT emit `PNAMES_COMPLEX`**; when coverage is impacted by an un‑compilable pattern, Compose treats it as unknown gating and may emit `REGEX_COMPILE_ERROR{context:"coverage"}` and `AP_FALSE_INTERSECTION_APPROX{reason:"regexCompileError"}` as applicable.  
  **Payload conformance:** regex diagnostics’ `details` **MUST** satisfy §19.1 (regex payloads) and use the JSON‑unescaped `patternSource`.
...
* `enumerate()` **MUST** be provided **iff** the **global must‑cover intersection** is a **provably finite** set built exclusively from:
  (a) named `properties`, and/or (b) **synthetic** exact‑literal‑alternatives (Glossary) introduced by the §7 rewrite (`PNAMES_REWRITE_APPLIED`).
  ...
  **Note (non‑normative).** Enumeration intentionally excludes finite sets contributed solely by user‑authored
  `patternProperties` even when written as exact‑literal alternatives. This conservatism limits the proof surface
  across `allOf`. Core behavior does **not** enumerate those cases.
  ...
  **Provenance (normative).** When `enumerate()` is provided, `provenance` **MUST** be present and list the
  ...
  whenever §7 injected synthetic patterns were used (`PNAMES_REWRITE_APPLIED`). `provenance` remains coarse and
 ...
 * **Objects — `additionalProperties` exactness**
  ...
  **Payload (normative).** `details:{ sourceKind:'patternProperties'|'propertyNamesSynthetic', patternSource?:string }`.
...
* **Objects — other**
  ...
  * Conjunct `patternProperties`; record overlaps in `diag.overlaps.patterns`. Respect `disablePatternOverlapAnalysis`.
    **Normative (disabled shape).** When `disablePatternOverlapAnalysis === true`, implementations **MUST** set
    `diag.overlaps` to `undefined` at the node (do not emit empty or partial results). This guarantees a stable,
    deterministic output shape across runs and options.
...
* Numeric/object/array bounds:
  ...
  * **Pattern vs `propertyNames` (normative gating).**
    ...
    4. There is **presence pressure** (`effectiveMinProperties > 0` **or** there exists `r ∈ required` with `r ∉ E`).
...
* **Normative (observability & locus):** in score‑only, implementations **MUST invoke the RNG exactly once** and record the resulting float as `tiebreakRand` **even when `|T| = 1`** (not used for selection, but recorded for auditability). `diag.chosenBranch`, `diag.scoreDetails`, `diag.budget` and `diag.overlap` refer to the **branch operator at the Compose entry `canonPath`**. When Compose is invoked on a non‑branch node these fields **MUST be omitted** (field absent). When invoked on a branch node, `diag.scoreDetails` **MUST** be present and include the fields below.
...
* External `$ref`: default **warn** then attempt generation without resolving remote refs (no network I/O).
  ...
  **Metrics (normative):** When this skip path is taken, implementations **MUST** set `diag.metrics.validationsPerRow = 0` for the affected row(s), aligning with §1 Acceptance.
...
`guards.maxGeneratedNotNesting`, `patternWitness.alphabet`, `patternWitness.maxCandidates`, `patternWitness.maxLength`,
`rational.decimalPrecision`, `rational.fallback`, `rational.maxRatBits`, `rational.maxLcmBits`, `rational.qCap`, `repair.mustCoverGuard`
...
**Severity routing (normative).** Coverage‑time `REGEX_COMPLEXITY_CAPPED{context:'coverage'}` and
`REGEX_COMPILE_ERROR{context:'coverage'}` **MUST** be recorded as **non‑fatal** entries in `diag.warn` unless escalated by another rule (see §8).
...
export function compose(schema: any, opts?: ComposeOptions): {
  ...
  // Compose-time caps only. **Normative contract mirrors §8 Responsibilities**:
  // subset of {'COMPLEXITY_CAP_ONEOF','COMPLEXITY_CAP_ANYOF','COMPLEXITY_CAP_ENUM',
  // 'COMPLEXITY_CAP_CONTAINS','COMPLEXITY_CAP_SCHEMA_SIZE'}; **MUST** be de-duplicated and
  // sorted in UTF-16 ascending order before export; **MUST NOT** include 'COMPLEXITY_CAP_PATTERNS'.
  caps?: string[];
  };
};
```
