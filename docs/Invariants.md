...
- For schema locations classified as `G_valid` by the canonical SPEC (§6), the generator is responsible for structural AJV validity by construction: required keys and cardinality constraints are satisfied in Generate, and any structural repairs observed under `G_valid` are treated as regressions and surfaced via `diag.metrics.gValid_*` counters. Within this zone, Repair is expected to perform only low-impact adjustments (for example numeric/format nudges or `uniqueItems` deduplication) and MUST NOT be relied upon to add missing required properties or whole sub-objects; any such structural action counts against the G_valid metrics and is treated as a contract violation to investigate.
...
- Tier and policy effects are observable: each action is classified into Tier‑0/Tier‑1/Tier‑2/Tier‑3 per the SPEC table, policy blocks are surfaced via `REPAIR_TIER_DISABLED` (with `details.{keyword,requestedTier,allowedMaxTier,reason}`), and when metrics are enabled the engine aggregates deterministic counters (`diag.metrics.repair_tier1_actions`, `repair_tier2_actions`, `repair_tier3_actions`, `repair_tierDisabled`, `repair_reverted_no_progress`, plus `repairActionsPerRow` and `repairPassesPerRow`) for use by reporters and acceptance tests.
