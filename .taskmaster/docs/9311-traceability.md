...
- **9311.9311004 – Add fixtures and tests for coverage diff behavior**  
  Covers: [KR4], [DEL3], [DOD2], [DOD4], [TS2], [TS3], [TS4]. Status: covered (adds focused unit tests for the core coverage diff module over synthetic CoverageReports, keeps the reporter façade wired to the core API, and extends CLI tests to exercise `foundrydata coverage diff` end-to-end on JSON fixtures, validating target classification, metric deltas, regressions and newly uncovered targets while keeping diff behavior within the compatibility envelope of coverage-report/v1).
