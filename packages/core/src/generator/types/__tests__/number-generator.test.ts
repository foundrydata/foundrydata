...
function numberFromSchema(schema: NumberSchemaConstraints) {
  ...
  if (iMin > iMax) {
    throw new Error(`Invalid range: no valid multiples of ${step} between ${min} and ${max}`);
  }
  
  return fc.integer({ min: iMin, max: iMax })
    .map(i => Number((i * step).toFixed(digits)));
}

// Generator for continuous numbers with simple exclusive boundaries (Case 2: no multipleOf)
function openDouble(min: number, max: number) {
...
function decimalArb(min: number, max: number, step: number) {
  const digits = decimalPlaces(step);
  const iMin = Math.ceil(min / step);
  const iMax = Math.floor(max / step);
...
function isMultipleOf(value: number, multipleOf: number): boolean {
  ...
  const tol = ulp(value) + Math.abs(k) * ulp(multipleOf) + Math.abs(value) * 1e-15;
...
describe('NumberGenerator', () => {
  ...
  describe('supports', () => {
      ...
      fc.assert(
        fc.property(
          fc.record({
            ...
            minimum: fc.option(fc.double({ noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            maximum: fc.option(fc.double({ noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            multipleOf: fc.option(decimalArb(0.01, 10, 0.01), { nil: undefined }),
            exclusiveMinimum: fc.option(fc.double({ min: -100, max:  50, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            exclusiveMaximum: fc.option(fc.double({ min:  -50, max: 100, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            enum: fc.option(fc.array(fc.double({ noNaN: true, noDefaultInfinity: true }), { minLength: 1 }), { nil: undefined }),
            // Note: minLength: 1 for quality, though JSON Schema spec only says enum SHOULD be non-empty
            const: fc.option(fc.double({ noNaN: true, noDefaultInfinity: true }), { nil: undefined })
          }),
          (schemaProps) => {
    ...
    it('should not support non-number schemas', () => {
      fc.assert(
        fc.property(
          fc.oneof(
            fc.record({ type: fc.constantFrom('string', 'boolean', 'object', 'array', 'integer') }),
    ...
    it('should not support draft-04 boolean exclusive bounds (Draft-07+ only)', () => {
      const schemas = [
        ...
        { type: 'number' as const, minimum: 5, maximum: 15, exclusiveMinimum: false as any, exclusiveMaximum: true as any },
      ];
  ...
  describe('generate', () => {
    ...
    it('should generate values from enum when provided', () => {
      fc.assert(
        fc.property(
          fc.array(fc.double({ noNaN: true, noDefaultInfinity: true }), { minLength: 1, maxLength: 10 }),
          fc.integer({ min: 1, max: 100000 }),
          (enumValues, seed) => {
    ...
    it('should generate same values with same seed', () => {
      fc.assert(
        fc.property(
          fc.integer({ min: 1, max: 100000 }),
          fc.record({
            minimum: fc.option(fc.double({ min: -50, max: 0, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            maximum: fc.option(fc.double({ min: 0, max: 50, noNaN: true, noDefaultInfinity: true }), { nil: undefined })
          }),
    ...
    it('should handle edge case scenarios', () => {
      fc.assert(
        fc.property(
          fc.record({
            minimum: fc.option(fc.double({ min: -100, max: 0, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            maximum: fc.option(fc.double({ min: 1, max: 100, noNaN: true, noDefaultInfinity: true }), { nil: undefined })
          ...
          (constraintProps, seed) => {
            ...
            if (constraintProps.minimum !== undefined && constraintProps.maximum !== undefined) {
            ...
            if (result.isOk()) {
              if (typeof constraintProps.minimum === 'number') {
                expect(result.value).toBeGreaterThanOrEqual(constraintProps.minimum);
              }
              if (typeof constraintProps.maximum === 'number') {
                expect(result.value).toBeLessThanOrEqual(constraintProps.maximum);
              }
            }
          }
        )
        ...
        fc.property(
          ...
          (constraints, seed) => {
            ...
            if (result.isOk()) {
              ...
              expect(isMultipleOf(result.value, constraints.multipleOf)).toBe(true);
            }
          }
        )
      );
      ...
      fc.assert(
        fc.property(
          ...
          (seed1, seed2) => {
            ...
            const schema: NumberSchema = { type: 'number', minimum: 0, maximum: 1000 };
            
            const context1 = createGeneratorContext(schema, formatRegistry, { seed: seed1 });
            const context2 = createGeneratorContext(schema, formatRegistry, { seed: seed2 });
    ...
    it('should error when no multipleOf fits in range', () => {
      const schema: NumberSchema = { type: 'number', minimum: 1, maximum: 1.5, multipleOf: 2 };
      const ctx = createGeneratorContext(schema, formatRegistry, { seed: 1 });
      expect(generator.generate(schema, ctx).isErr()).toBe(true);
    });

    it('should error when exclusiveMinimum >= exclusiveMaximum', () => {
      const schema: NumberSchema = { type: 'number', exclusiveMinimum: 10, exclusiveMaximum: 10 };
      const ctx = createGeneratorContext(schema, formatRegistry, { seed: 1 });
      expect(generator.generate(schema, ctx).isErr()).toBe(true);
    });
    ...
    it('should handle enum with additional constraints', () => {
      fc.assert(
        fc.property(
          fc.array(fc.double({ noNaN: true, noDefaultInfinity: true }), { minLength: 3, maxLength: 10 }),
    ...
    it('should reject NaN and Infinity values in enum', () => {
      const specialValues = [Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY];
  ...
  describe('validate', () => {
    it('should validate number values correctly', () => {
      fc.assert(
        fc.property(
          fc.double({ noNaN: true, noDefaultInfinity: true }),
          fc.record({
            minimum: fc.option(fc.double({ min: -100, max: 100, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            maximum: fc.option(fc.double({ min: -100, max: 100, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
          ...
          (value, constraintProps) => {
            ...
            if (typeof constraintProps.minimum === 'number' && value < constraintProps.minimum) {
              shouldBeValid = false;
            }
            if (typeof constraintProps.maximum === 'number' && value > constraintProps.maximum) {
              shouldBeValid = false;
            }
      ...
      fc.assert(
        fc.property(
          fc.array(
            ...
            const outside = fc.double({ min: -1000, max: 1000, noNaN: true, noDefaultInfinity: true })
    ...
    it('should validate enum with additional constraints', () => {
      fc.assert(
        fc.property(
          fc.array(fc.double({ min: 0, max: 100, noNaN: true, noDefaultInfinity: true }), { minLength: 3, maxLength: 5 }),
          fc.double({ min: -50, max: 50, noNaN: true, noDefaultInfinity: true }),
          ...
          (enumValues, testValue, outsideValue) => {
            ...
            const validEnumValue = enumValues.find(val => val >= 10 && val <= 90);
            const invalidEnumValue = enumValues.find(val => val < 10 || val > 90);
  ...
  describe('getExamples', () => {
    ...
    it('should return schema examples when available', () => {
      fc.assert(
        fc.property(
          fc.array(fc.double({ noNaN: true, noDefaultInfinity: true }), { minLength: 1, maxLength: 5 }),
          (schemaExamples) => {
            const schema: NumberSchema = { type: 'number', examples: schemaExamples };
            const examples = generator.getExamples(schema);
    ...
    it('should generate constraint-based examples when no explicit examples', () => {
      fc.assert(
        fc.property(
          fc.record({
            minimum: fc.option(fc.double({ min: -10, max: 0, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
            maximum: fc.option(fc.double({ min: 0, max: 10, noNaN: true, noDefaultInfinity: true }), { nil: undefined }),
          ...
          (constraintProps) => {
            ...
            examples.forEach((example) => {
              ...
              if (typeof constraintProps.multipleOf === 'number') {
                expect(isMultipleOf(example, constraintProps.multipleOf)).toBe(true);
              }
            });
          }
        )
    ...
    it('getExamples: handles very small multipleOf', () => {
      const schema: NumberSchema = { type: 'number', minimum: 0, maximum: 1e-9, multipleOf: 1e-12 };
      const examples = generator.getExamples(schema);
    ...
    it('getExamples: should return unique values', () => {
      const schema: NumberSchema = { type: 'number', minimum: 0, maximum: 10, multipleOf: 0.5 };
  ...
  describe('integration tests', () => {
      ...
      fc.assert(
        fc.property(
          fc.oneof(
            ...
            fc.double({ min: -10, max: 10, noNaN: true, noDefaultInfinity: true }).chain(min => 
              fc.double({ min: min, max: min + 20, noNaN: true, noDefaultInfinity: true }).map(max => ({
                minimum: min,
                maximum: max
              }))
            ),
            ...
            fc.record({
              enum: fc.array(fc.double({ min: -100, max: 100, noNaN: true, noDefaultInfinity: true }), { minLength: 1, maxLength: 5 })
            }),
          ...
          (schemaProps, seed) => {
            const schema = cleanSchema({
              type: 'number',
              ...('minimum' in schemaProps && typeof schemaProps.minimum === 'number' ? { minimum: schemaProps.minimum } : {}),
              ...('maximum' in schemaProps && typeof schemaProps.maximum === 'number' ? { maximum: schemaProps.maximum } : {}),
              ...('multipleOf' in schemaProps && typeof schemaProps.multipleOf === 'number' ? { multipleOf: schemaProps.multipleOf } : {}),
              ...('enum' in schemaProps && Array.isArray(schemaProps.enum) ? { enum: schemaProps.enum } : {})
            });
            const context = createGeneratorContext(schema, formatRegistry, { seed });
    ...
    it('exclusiveMaximum exactly on a multiple should exclude it', () => {
      const s = { type: 'number', exclusiveMaximum: 20, multipleOf: 5 } as NumberSchema;
  ...
  describe('comprehensive constraints coverage', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (exclusiveMin, exclusiveMax, seed) => {
            ...
            const [min, max] = exclusiveMin < exclusiveMax 
    ...
    it('should generate valid enum values', () => {
      fc.assert(
        fc.property(
          fc.array(fc.double({ min: -100, max: 100, noNaN: true, noDefaultInfinity: true }), { minLength: 1, maxLength: 10 }),
          fc.integer({ min: 1, max: 100000 }),
          (enumValues, seed) => {
            const schema: NumberSchema = { type: 'number', enum: enumValues };
            const context = createGeneratorContext(schema, formatRegistry, { seed });
    ...
    it('metamorphic: generated value +/- half-step should violate multipleOf (if in-bounds)', () => {
      fc.assert(fc.property(
        ...
        (props, seed) => {
          ...
          if (v + d <= 10) expect(generator.validate(v + d, schema)).toBe(false);
          if (v - d >= 0)  expect(generator.validate(v - d, schema)).toBe(false);
        }
      ));
    });
  ...
  describe('couverture fonctionnelle complète', () => {
    describe('1. schémas impossibles/contradictoires', () => {
        ...
        fc.assert(
          fc.property(
            ...
            (val1, val2, seed) => {
              ...
              expect(generator.validate((minimum + maximum) / 2, schema)).toBe(false);
            }
          )
        );
      });
        ...
        fc.assert(
          fc.property(
            ...
            (val1, val2, seed) => {
              ...
              const schema: NumberSchema = { type: 'number', exclusiveMinimum, exclusiveMaximum };
              const context = createGeneratorContext(schema, formatRegistry, { seed });
      ...
      it('should handle contradictory minimum + exclusiveMinimum', () => {
        ...
        testCases.forEach(constraintProps => {
          ...
          if ('exclusiveMinimum' in constraintProps && 'minimum' in constraintProps) {
            const min = Math.min(constraintProps.exclusiveMinimum!, constraintProps.minimum!);
            expect(generator.validate(min, schema)).toBe(false);
          }
          if ('exclusiveMaximum' in constraintProps && 'maximum' in constraintProps) {
            const max = Math.max(constraintProps.exclusiveMaximum!, constraintProps.maximum!);
    ...
    describe('3. enum/const combinés aux autres contraintes', () => {
      it('should handle enum with values outside bounds', () => {
        fc.assert(
          fc.property(
            ...
            fc.double({ min: 60, max: 100, noNaN: true, noDefaultInfinity: true }),
            fc.integer({ min: 1, max: 100000 }),
            (minBound, maxBound, seed) => {
              ...
              enumValues.forEach(enumValue => {
                const isInBounds = enumValue >= minBound && enumValue <= maxBound;
      ...
      it('should handle const with additional constraints', () => {
        ...
        testCases.forEach(({ shouldBeValid, ...schemaProps }) => {
          ...
          expect(generator.validate(schemaProps.const, schema)).toBe(shouldBeValid);
        });
      });
    ...
    describe('4. exemples générés automatiquement', () => {
      it('should generate compliant examples when no explicit examples', () => {
        const schemas = [
          ...
          { type: 'number' as const, exclusiveMinimum: 0, exclusiveMaximum: 100 },
      ...
      it('should limit example list size', () => {
        const schema: NumberSchema = { type: 'number', minimum: -1000, maximum: 1000 };
        const examples = generator.getExamples(schema);
    ...
    describe('5. cas IEEE-754 -0', () => {
      ...
      it('should distinguish -0 and +0 when appropriate', () => {
        ...
        expect(Object.is(0, -0)).toBe(false);
        expect(-0 === 0).toBe(true); // But === considers them equal
    ...
    describe('6. formes draft-04 vs modernes pour exclusive*', () => {
          ...
          try {
            fc.assert(
              fc.property(
                numberFromSchema(schema),
                (x) => {
                  if (schema.exclusiveMinimum !== undefined) expect(x).toBeGreaterThan(schema.exclusiveMinimum);
                  if (schema.minimum !== undefined) expect(x).toBeGreaterThanOrEqual(schema.minimum);
                  if (schema.exclusiveMaximum !== undefined) expect(x).toBeLessThan(schema.exclusiveMaximum);
                  if (schema.maximum !== undefined) expect(x).toBeLessThanOrEqual(schema.maximum);
                  expect(isMultipleOf(x, schema.multipleOf!)).toBe(true);
                  const rem = x % schema.multipleOf!;
                  expect(Math.abs(rem) < 1e-10 || Math.abs(rem - schema.multipleOf!) < 1e-10).toBe(true);
                }
              ),
              { numRuns: 20 }
            );
            ...
            if (error instanceof Error && error.message.includes('Invalid range')) {
      ...
      it('should handle mixed minimum and exclusiveMinimum', () => {
        ...
        const testCases = [
          { minimum: 5, exclusiveMinimum: 10, expectedMin: 10, inclusive: false },
          { minimum: 15, exclusiveMinimum: 10, expectedMin: 15, inclusive: true }
        ];

        testCases.forEach(({ minimum, exclusiveMinimum, expectedMin, inclusive }) => {
          const schema: NumberSchema = { type: 'number', minimum, exclusiveMinimum };
      ...
      it('should honor tighter of maximum and exclusiveMaximum', () => {
        const s: NumberSchema = { type: 'number', maximum: 10, exclusiveMaximum: 100, multipleOf: 1 };
        const ctx = createGeneratorContext(s, formatRegistry, { seed: 1 });
        const r = generator.generate(s, ctx);
        if (r.isOk()) {
      ...
      it('should honor tighter of minimum and exclusiveMinimum', () => {
        const s: NumberSchema = { type: 'number', minimum: 5, exclusiveMinimum: 0, multipleOf: 1 };
        const ctx = createGeneratorContext(s, formatRegistry, { seed: 1 });
        const r = generator.generate(s, ctx);
        if (r.isOk()) {
    ...
    describe('6.5. tests de régression pour bugs classiques', () => {
      ...
      it('should handle very small multipleOf values without precision loss', () => {
        ...
        smallSteps.forEach(step => {
          const schema: NumberSchema = { type: 'number', multipleOf: step, minimum: 0, maximum: step * 10 };
          const context = createGeneratorContext(schema, formatRegistry, { seed: 42 });
      ...
      it('should handle large multipleOf intervals efficiently', () => {
        ...
        const values: number[] = [];
        for (let i = 0; i < 10; i++) {
          const ctx = createGeneratorContext(schema, formatRegistry, { seed: i });
      ...
      it('should generate meaningful edge values with improved distribution', () => {
        ...
        for (let i = 0; i < 20; i++) {
          const context = createGeneratorContext(schema, formatRegistry, { seed: i, scenario: 'edge' });
          const result = generator.generate(schema, context);
          if (result.isOk()) {
        ...
        const foundExpected = expectedEdgeCases.filter(expected => edgeValues.includes(expected));
      ...
      it('should handle Number.EPSILON edge cases correctly', () => {
        ...
        const hasNegativeEpsilon = edgeValues.some(v => v === -Number.EPSILON);
    ...
    describe('7. extrêmes et précision', () => {
      it('should handle extreme bounds near JS limits', () => {
        const extremeSchemas = [
          ...
          { exclusiveMinimum: -Number.MAX_SAFE_INTEGER, exclusiveMaximum: Number.MAX_SAFE_INTEGER }
        ];

        extremeSchemas.forEach(schema => {
          const typedSchema: NumberSchema = { type: 'number', ...schema };
          const context = createGeneratorContext(typedSchema, formatRegistry, { seed: 42 });
      ...
      it('should handle narrow ranges without invalid delta additions', () => {
        ...
        narrowRanges.forEach(({ min, max }) => {
          const width = max - min;
          const schema: NumberSchema = { type: 'number', minimum: min, maximum: max };
          const context = createGeneratorContext(schema, formatRegistry, { scenario: 'edge', seed: 42 });
